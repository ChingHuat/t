    ...
    <div className="space-y-6 animate-in fade-in duration-300 max-w-4xl mx-auto">
      ...
      <div className="relative" ref={containerRef}>
        <div className="relative">
          <input 
            ...
            className="w-full pl-10 pr-10 py-3.5 bg-slate-900 border border-slate-800 rounded-xl shadow-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-sm font-black tracking-tight text-slate-100 placeholder:text-slate-600"
          />
        ...
        {searchResults.length > 0 && (
          <div className="absolute top-full left-0 right-0 z-50 mt-2 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl overflow-hidden animate-in slide-in-from-top-2">
      ...
      {loading && (
        <div className="flex flex-col items-center py-16">
          ...
          <p className="mt-4 text-[10px] font-black text-slate-600 uppercase tracking-[0.3em]">Acquiring Link</p>
        </div>
      )}

      {data && (
        <div className="space-y-4 pt-2">
          <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex items-center justify-between gap-4 shadow-xl">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 mb-2">
                <span className="px-1.5 py-0.5 bg-slate-800 text-[9px] font-[1000] text-white rounded uppercase tracking-tighter border border-slate-700">STATION {data.busStopCode}</span>
                {weather && <WeatherIndicator weather={weather} />}
              </div>
              <h3 className="text-base font-[1000] text-slate-100 truncate leading-none uppercase tracking-tighter">{data.busStopName}</h3>
              <p className="text-[9px] font-bold text-slate-500 uppercase mt-1.5 leading-none tracking-wide">{data.roadName}</p>
            </div>
            <button 
              onClick={() => toggleFavorite({ code: data.busStopCode, name: data.busStopName, road: data.roadName })}
              className={`p-3.5 rounded-xl transition-all shrink-0 active:scale-95 border shadow-lg ${isFavorited ? 'bg-red-600 border-red-500 text-white shadow-red-900/20' : 'bg-slate-800 text-slate-500 border-slate-700'}`}
            >
