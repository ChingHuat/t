  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 max-w-5xl mx-auto">
      ...
      <div className="md:mb-2">
        ...
        <p className="text-[9px] font-black text-emerald-500 dark:text-emerald-400 uppercase tracking-[0.2em] mt-0.5">Preferences & Connectivity</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className={`p-4 rounded-xl border flex items-center justify-between transition-colors h-full ${
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Core Engine</p>
              <p className={`text-sm font-black ${statusLabel === 'online' ? 'text-emerald-700 dark:text-emerald-400' : statusLabel === 'offline' ? 'text-red-700 dark:text-red-400' : 'text-slate-600 dark:text-slate-400'}`}>
                {statusLabel === 'checking' ? 'Checking Link...' : statusLabel === 'online' ? 'Proxy Online' : 'Service Down'}
              </p>
            </div>
          </div>
        ...
        <div className={`p-4 rounded-xl border transition-all h-full flex items-center gap-4 ${isConfigured ? 'bg-emerald-50/50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-900/30' : 'bg-amber-50/50 dark:bg-amber-900/10 border-amber-100 dark:border-amber-900/30'}`}>
          <div className={`p-2 rounded-lg shrink-0 ${isConfigured ? 'bg-emerald-500 text-white' : 'bg-amber-500 text-white'}`}>
            {isConfigured ? <ShieldCheck className="w-5 h-5" /> : <AlertCircle className="w-5 h-5" />}
          </div>
          <div className="min-w-0">
            <h3 className={`text-sm font-black tracking-tight ${isConfigured ? 'text-emerald-900 dark:text-emerald-300' : 'text-amber-900 dark:text-amber-300'}`}>
              Alerts: {isConfigured ? 'Sync Ready' : 'Standby'}
            </h3>
            <p className={`text-[10px] font-bold truncate ${isConfigured ? 'text-emerald-700/70 dark:text-emerald-500/70' : 'text-amber-700/70 dark:text-amber-500/70'}`}>
              {isConfigured ? `Delivering to @${telegramId.replace('@', '')}` : 'Set Telegram ID to enable alerts'}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="bg-white dark:bg-slate-900 p-5 md:p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-5">
            <div className="flex items-center gap-2.5">
              ...
              <h3 className="text-sm font-black text-slate-800 dark:text-slate-100 uppercase tracking-widest">Link Identity</h3>
            </div>
            
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest ml-1">Telegram Handle</label>
                <div className="flex flex-col sm:flex-row gap-2">
                  <input 
                    ...
                    className="flex-1 px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all text-sm font-bold text-slate-800 dark:text-slate-100 placeholder:text-slate-300"
                  />
                  <button 
                    onClick={handleSave}
                  ...
                  >
                    {isSaved ? <CheckCircle2 className="w-4 h-4" /> : 'Update'}
                  </button>
                </div>
              </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-slate-100/30 dark:bg-slate-900/30 p-5 md:p-6 rounded-2xl border border-transparent dark:border-slate-800 h-full">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <h3 className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest">Protocol Help</h3>
            </div>
            <ul className="space-y-4">
              {[
                { id: '1', text: 'If Network Error occurs, check if your ISP or AdBlock is filtering API traffic.' },
                { id: '2', text: 'Verify the 5-digit station code against official LTA signage.' },
                { id: '3', text: 'Missing alerts? Ensure you have typed /start in the @SGBusProBot chat.' }
              ].map(item => (
      ...
      <div className="text-center pt-8 md:pt-12">
        <p className="text-[9px] text-slate-400 dark:text-slate-600 font-black uppercase tracking-[0.3em]">
          SG Bus Arrival Pro • v1.4.2 • Build 2404
        </p>
      </div>
    </div>
  );
